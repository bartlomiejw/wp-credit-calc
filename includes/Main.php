...
final class Main
{
     ...
     * @var string
     */
    public const PREFIX = 'creditcalc';
    ...
    public static function uninstall_plugin()
    {
        ...
        (new \CreditCalc\Migrations())->cleanUp(self::PREFIX, $settings);
    }
    ...
    public function run()
    {
        ...
        if (defined('WP_CLI') && \WP_CLI) {
            $this->container['cli'] = new \CreditCalc\CliLoader(self::PREFIX);
        }
    ...
    {
        (new \CreditCalc\Migrations())->run(self::PREFIX, $this->VERSION);
    ...
    {
        // initialize assets
        $this->container['assets'] = new \CreditCalc\Assets(self::PREFIX);

        // initialize the various loader classes
        if ($this->is_request('admin')) {
            $ctx = new \CreditCalc\AdminLoader(self::PREFIX);
            $this->container['admin'] = $ctx;
        }

        if ($this->is_request('frontend')) {
            $this->container['frontend'] = new \CreditCalc\FrontendLoader(self::PREFIX);
        }

        if ($this->is_request('ajax')) {
//             $this->container['ajax'] =  new \CreditCalc\AjaxLoader(self::PREFIX);
        }

        // finally load api routes
        $this->container['api'] = new \CreditCalc\ApiRoutes(self::PREFIX);
    }
